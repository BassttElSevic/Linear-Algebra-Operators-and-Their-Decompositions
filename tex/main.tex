...
\newcommand{\bookauthor}{Basstt\_ElSevic}
...
\begin{tikzpicture}[remember picture, overlay]
  ...
  \fill[accent] ($(current page.north west) + (16mm, 0)$) rectangle
    ($(current page.south west) + (19mm, 0)$);
  % 右上角小方块装饰
  \fill[accent!25] ($(current page.north east) + (-30mm, -18mm)$) rectangle
    ($(current page.north east) + (-24mm, -24mm)$);
  \fill[thmred!60] ($(current page.north east) + (-22mm, -18mm)$) rectangle
...
\vfill
\begin{flushleft}
  {\sffamily\large\color{inkblue}\bookauthor}\\[3mm]
...
本书是作者自编的线性代数笔记，主题是有限维向量空间上的\emph{算子}以及它的各种分解：
...
\begin{jdefn*}{定义框}{}
青蓝色块给出概念的定义，以及为了让定义站得住脚所需要的最少解释。
\end{jdefn*}
...
\begin{jidea}{理解}
紫色块是对刚才那段形式化内容的翻译，回答"这件事到底在说什么"。
\end{jidea}

\begin{jinsight}
金色块是一章或一节的要点：如果只记一句话，就记这一句。
\end{jinsight}

未加色块的部分是叙述性的正文，负责把这些结论串成一条线。
每一"部分"的开头都有一页蓝色导读，说明该部分研究什么、用到哪些结构。
